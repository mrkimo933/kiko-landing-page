...
# Read backwards to find the latest USER_INPUT containing the HTML
for line in reversed(lines):
    try:
        data = json.loads(line)
        if data.get('type') == 'USER_INPUT' and '<!DOCTYPE html>' in data.get('content', '') and 'Footer' in data.get('content', ''):
            ...
            if match:
                ...
                print(f"Successfully updated! Length: {len(html)}")
                break
    except:
        continue
